...
Процедура ВыполнитьРегламентныеДействия()
	
	Логин = "sadym_anton@mail.ru";
	Пароль = "Tornado32989";
	ВыполнитьАвторизациюПриНеобходимости(Логин, Пароль);
	...
	Для каждого СтрокаОрганизации Из ОсновнойМодуль.ЭДО_Модуль_ТаблицаОрганизацийПользователя() Цикл
		...
		ЗаписьЖурналаРегистрации("Диадок", УровеньЖурналаРегистрации.Информация,,, "Производится обновление данных по организации " + ОргНаименование);
		
		Попытка
			ОсновнойМодуль.ЭДО_Модуль_ОбработатьНовыеСобытияДокументов(СтрокаОрганизации);
		Исключение
			ОсновнойМодуль.ЭДО_Служебные_СообщитьОбОшибке("При обновлении данных по организации " + ОргНаименование + " произошли ошибки:
				|"+ОписаниеОшибки());
		КонецПопытки;
...
Функция ПолучитьЗаполненныйОбъектДокумента1С(Параметры)
	...
	ВходящийДокументОбъект.ВидДокумента = ПолучитьВидДокумента(ДокументДД.ТипДокумента);
	ВходящийДокументОбъект.Проект = Константы.Диадок_ПроектПоУмолчанию.Получить();
	ВходящийДокументОбъект.Наименование = ДокументДД.НаименованиеДокумента;
	ВходящийДокументОбъект.Заголовок = ДокументДД.НаименованиеДокумента;
	ВходящийДокументОбъект.Организация = ПолучитьОрганизацию(ДокументДД);
	ВходящийДокументОбъект.Отправитель = ПолучитьКорреспондента(ДокументДД);
	ВходящийДокументОбъект.ВопросДеятельности = Константы.Диадок_ВопросДеятельностиПоУмолчанию.Получить();
	ВходящийДокументОбъект.Комментарий = "Из Диадока. Комментарий отправителя: " + ДокументДД.КомментарийДокумента;
	ВходящийДокументОбъект.ДатаСоздания = ДокументДД.ДатаСозданияДокумента;
	ВходящийДокументОбъект.ИсходящийНомер = ДокументДД.НомерДокумента;
	ВходящийДокументОбъект.ИсходящаяДата = ДокументДД.ДатаДокумента;
	
	ВходящийДокументОбъект.Записать();
	
	имяВременногоФайла = ПолучитьИмяВременногоФайла("pdf");
	двоичныеДанныеФайла = ДокументДД.ХранилищеПечатнойФормы.Получить();
	Если двоичныеДанныеФайла = Неопределено Тогда
		двоичныеДанныеФайла = ДокументДД.ХранилищеКонтента.Получить();
	КонецЕсли;
	...
	РаботаСФайламиВызовСервера.СоздатьФайлСВерсией(
		...
		ДокументДД.НаименованиеДокумента,
		"pdf",
		ДокументДД.ДатаСозданияДокумента,
		ДокументДД.ДатаСозданияДокумента,
	...
	Возврат ВходящийДокументОбъект;
...
Функция НайтиСопоставлениеДокумента(Параметры)
	...
	// Заглушка, ДокументСсылка - Справочник.ВходящийДокумент, к которому добавляем файл
	ДокументСсылка = Справочники.ВходящиеДокументы.НайтиПоНаименованию("Входящее письмо");
	Логин = "skopin.andrew.b@gmail.com";
	Пароль = "rebootabcd";
	
	// Авторизоваться и получить данные контента из Диадока, а именно: имя файла, размер, сам файл
	//ВыполнитьАвторизациюПриНеобходимости(Параметры.ДокументДД.BoxId, Логин, Пароль);
	ДокументПрямогоAPI = ОсновнойМодуль.API_GetDocument(Параметры.ДокументДД.BoxId, Параметры.ДокументДД.LetterId, Параметры.ДокументДД.DocumentId);
	имяФайлаКонтента = ПолучитьИмяВременногоФайла();
	Base64Значение(ДокументПрямогоAPI.Content.Data).Записать(имяФайлаКонтента);
	имяРасширениеФайла = ПолучитьИмяРасширение(ДокументПрямогоAPI.FileName);
	
	РаботаСФайламиВызовСервера.СоздатьФайлСВерсией(
		ДокументСсылка,
		имяРасширениеФайла.Имя,
		имяРасширениеФайла.Расширение,
		Ticks_2_Date(ДокументПрямогоAPI.CreationTimestampTicks),
		Ticks_2_Date(ДокументПрямогоAPI.CreationTimestampTicks),
		ДокументПрямогоAPI.Content.Size,
		имяФайлаКонтента,
		"", Ложь);
	
	УдалитьФайлы(имяФайлаКонтента);
	
	// Вызвать метод формирования PDF-представления в Диадоке и также добавить в файл
	ДвоичныеДанныеПредставления = ОсновнойМодуль.ЭДО_ПолучитьПечатнуюФормуДокумента(Параметры.ДокументДД.BoxId, Параметры.ДокументДД.LetterId, Параметры.ДокументДД.DocumentId, 10);
	
	Если ДвоичныеДанныеПредставления <> Неопределено Тогда
		
		имяФайлаКонтента = ПолучитьИмяВременногоФайла();
		ДвоичныеДанныеПредставления.Записать(имяФайлаКонтента);
		
		РаботаСФайламиВызовСервера.СоздатьФайлСВерсией(
			ДокументСсылка,
			имяРасширениеФайла.Имя + "_представление",
			"pdf",
			ТекущаяДата(),
			ТекущаяДата(),
			ДвоичныеДанныеПредставления.Размер(),
			имяФайлаКонтента,
			"", Ложь);
...
Функция ПолучитьИмяРасширение(имяФайла)
	...
	Пока НЕ ПустаяСтрока(остСтрока) Цикл
		...
		накопленнаяДлина = накопленнаяДлина + поз;
		остСтрока = Сред(остСтрока, поз + 1);
		
	КонецЦикла;
	
	Если накопленнаяДлина = 0 Тогда
		накопленнаяДлина = СтрДлина(имяФайла) + 1;
	КонецЕсли;
	
	Возврат Новый Структура("Имя,Расширение", Лев(имяФайла, накопленнаяДлина - 1), Сред(имяФайла, накопленнаяДлина + 1));
	
КонецФункции


Процедура ВыполнитьАвторизациюПриНеобходимости(Логин, Пароль)
	
	ТокенДляBoxID = ПолучитьТокенДляBoxID("");
	
	Если ТокенДляBoxID = Неопределено И Логин <> "" Тогда
				
		ТокенДляBoxID = ОсновнойМодуль.DD_API_Gettoken("Пароль", Логин, Пароль);
		...
		ОсновнойМодуль.СоответствиеТокеновBoxID.Вставить("", ТокенДляBoxID);
...
Функция ПолучитьТокенДляBoxID(BoxID)
	...
	Если ОсновнойМодуль.СоответствиеТокеновBoxID = Неопределено Тогда
		ОсновнойМодуль.СоответствиеТокеновBoxID = Новый Соответствие;
	КонецЕсли;
	
	Результат = ОсновнойМодуль.СоответствиеТокеновBoxID.Получить(BoxID);
	
	Если Результат = Неопределено Тогда
		Результат = ОсновнойМодуль.СоответствиеТокеновBoxID.Получить("");
	КонецЕсли;
...
Функция ПолучитьВидДокумента(ТипДокументаСтрокой)
	
	Попытка
		типДокумента = Вычислить("Перечисления.Диадок_ТипыДокументов." + ТипДокументаСтрокой);
	Исключение
		типДокумента = Перечисления.Диадок_ТипыДокументов.Nonformalized;
	КонецПопытки;
	...
	|	соотв.ВидДокумента
	|из
	|	регистрсведений.Диадок_СоответствияТиповДокументов как соотв
	|где
	|	соотв.ТипДокумента = &Тип";
	...
	Иначе
		...
		Возврат Константы.Диадок_ВидДокументаПоУмолчанию.Получить();
		
	КонецЕсли;
	
КонецФункции

Функция ПолучитьОрганизацию(ДокументДД)
	
	Возврат ДокументДД.ОрганизацияДиадок.СвязанныйСправочник1;
	
КонецФункции

Функция ПолучитьКорреспондента(ДокументДД)
