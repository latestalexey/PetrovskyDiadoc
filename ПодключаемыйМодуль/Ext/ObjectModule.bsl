...
Процедура ВыполнитьАвторизациюПриНеобходимости(BoxID, Логин, Пароль)
	
	ТокенДляBoxID = ПолучитьТокенДляBoxID(ОсновнойМодуль.ЭДО_Служебные_АдресЯщикаКонтрагента(BoxID));
...
Функция ПолучитьЗаполненныйОбъектДокумента1С(Параметры)
	...
	Если СтруктураДанных.Тип = "XmlTorg12" ИЛИ СтруктураДанных.Тип = "UniversalTransferDocument" Тогда
		...
		//Если СтруктураДанных.Тип = "UniversalTransferDocument" И ДеревоКонтента.Документ.Функция = "СЧФ" Тогда
		//	Возврат ДокументОбъект; //отдельные СФ не отрабатываем
		//КонецЕсли;
		
		//ВерсияКонфигурации = Лев(Метаданные.Версия, 4);
		
		ДокументОбъект = Справочники.ВходящиеДокументы.СоздатьЭлемент();
		//Документы.ПоступлениеТоваровУслуг.СоздатьДокумент();
		
		ДокументОбъект.Заголовок				= "Входящее письмо" ;
		ДокументОбъект.ВидДокумента				= Справочники.ВидыВходящихДокументов.НайтиПоНаименованию("Входящий 1");
		
		ДокументОбъект.Отправитель				= СтруктураДанных.Контрагент;
		//ДокументОбъект.Партнер				= СтруктураДанных.Контрагент.Партнер;
		ДокументОбъект.Организация				= СтруктураДанных.Организация;			
		ДокументОбъект.ИсходящаяДата			= СтруктураДанных.Дата;
		ДокументОбъект.ИсходящийНомер			= СтруктураДанных.Номер;
		
		//ДокументОбъект.ВопросДеятельности		= Справочники.ВопросыДеятельности.НайтиПоКоду("00-000001");
		ДокументОбъект.Проект					= Справочники.Проекты.НайтиПоНаименованию("проект");
